...
export const registerUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // 1. Validar entrada (básico por ahora, se puede mejorar)
    if (!email || !password) {
    ...
    // 5. Guardar el usuario (en array en memoria por ahora)
    users.push(newUser);
    // MÁS ADELANTE: Aquí irá la lógica para guardar en el archivo users.json
...
export const getuserById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const item = users.find((u:User) => u.id === req.params.id);
    if (!item) {
...
export const updateUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const updatedItem = { ...users[itemIndex], ...req.body };
    users[itemIndex] = updatedItem;
